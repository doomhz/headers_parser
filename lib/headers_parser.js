(function() {
  ...
  HeadersParser = {
    ...
    getBrowser: function(headers) {
      var browser, browserVersion, _i, _len, _ref;
      if (headers["user-agent"]) {
        _ref = this.browserBlueprints;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          browser = _ref[_i];
          browserVersion = browser.regexp.exec(headers["user-agent"]);
          if (browserVersion) {
            return browserVersion[browser.name_index].replace(/\//g, " ");
          }
        }
      }
      return false;
    },
    getOS: function(headers) {
      var os, osVersion, _i, _len, _ref;
      if (headers["user-agent"]) {
        _ref = this.osBlueprints;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          os = _ref[_i];
          osVersion = os.regexp.test(headers["user-agent"]);
          if (osVersion) return os.os;
        }
      }
      return false;
    }
